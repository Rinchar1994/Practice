...
namespace MipsSimulator.Devices
{
    public class Register
    {
        ...
        static public void SetRegisterValue(string re)
        {
            re = re.Replace(" ", "");
            int temp = (Int32)Tools.CommonTool.StrToNum(TypeCode.Int32, re.Substring(0, 8), 16);
            MipsSimulator.Assembler.RunTimeCode.CodeIndex = temp == 0 ? 0 : (temp - 2048) / 4;
            re = re.Remove(0, 8);
            for(int i = 0; i < 20; i++)
            {
                Register.Res.Rows[i]["Value"] = "0x" + re.Substring(0, 8);
                re = re.Remove(0, 8);
            }
